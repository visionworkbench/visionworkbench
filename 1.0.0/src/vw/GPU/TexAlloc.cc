...
// Allocate
  ...
  if(recylingEnabled) {
	...
	cTex = *iter;
	if(cTex->format() == format && cTex->type() == type && cTex->width() == w && cTex->height() == h)
	  bestTex = cTex;	  
      }
      if(bestTex) {
	if(TEX_DEBUG_PRINT)
	...
	WriteToGPULog(buffer);
      }	  
    }
// if recycled not found, create new TexObj
  if(!recycledFound) {
    texObj = new TexObj(w, h, format, type);
    allocatedCount++;
...
// Create texBlock and return it
  TexBlock* texBlock = new TexBlock(texObj, w, h, 0, 0, realFormat, realType);
  return texBlock;		  
}

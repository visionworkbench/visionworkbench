...
#ifndef SimpleTransforms_H
#define SimpleTransforms_H
...
#include <vw/Image/ImageOperators.h>
...
namespace vw {
  ...
  // Image Packing - Gray -> RGBA
    ...
    GPUImage<PixelRGBA<ChannelT> > pack_gray_into_rgba(const GPUImage<PixelGray<ChannelT> >& image, int overlap) {
      // Static
      static GPUProgramSet programSet("", "SimpleTransforms/PackGrayIntoRGBA");
    ...
    GPUImage<PixelGray<ChannelT> > unpack_gray_from_rgba(const GPUImage<PixelRGBA<ChannelT> >& image, int overlap) {
      // Static
      static GPUProgramSet programSet_R("", "SimpleTransforms/SelectChannel-R");
      static GPUProgramSet programSet_G("", "SimpleTransforms/SelectChannel-G");
      static GPUProgramSet programSet_B("", "SimpleTransforms/SelectChannel-B");
      static GPUProgramSet programSet_A("", "SimpleTransforms/SelectChannel-A");
      static vector<int> fAttributes(1);
      fAttributes[0] = 4;
      // GLState - Setup
	  ((GPUImageBase&) image).rasterize_homography();	
      ...
      // 1. R-Channel	
      GPUProgram* program = programSet_R.get_program(vector<int>(), fAttributes, false);
      program->install();
      ...
      glEnd();
      // 2. G-Channel	
       program = programSet_G.get_program(vector<int>(), fAttributes, false);
      program->install();
      program->set_uniform_texture("image1", 0, image);
      ...
      glEnd();
      // 3. B-Channel	
      program = programSet_B.get_program(vector<int>(), fAttributes, false);
      program->install();
      program->set_uniform_texture("image1", 0, image);
      t_x1 = tImage_x1;
      t_x2 = tImage_x2 - overlap;
      ...
      glEnd();
      // 4. A-Channel	
      program = programSet_A.get_program(vector<int>(), fAttributes, false);
      program->install();
      program->set_uniform_texture("image1", 0, image);
      t_x1 = tImage_x1 + overlap;
      t_x2 = tImage_x2;

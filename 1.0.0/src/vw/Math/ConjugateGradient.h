...
namespace math{
  ...
  public:
    ...
    typename FuncT::domain_type operator()( FuncT const& func, 
      ...
      vw_out(DebugMessage) << "*** CharalambousStepSize ***" << std::endl;
      vw_out(DebugMessage) << "Initial val: " << cur_val << std::endl;
      vw_out(DebugMessage) << "Initial slope: " << cur_slope << std::endl;
      while( true ) {
        double new_stepsize = cur_stepsize + stepdelta;
        vw_out(DebugMessage) << "step: " << new_stepsize << std::endl;
        ...
        vw_out(DebugMessage) << "val: " << new_val << std::endl;
        vw_out(DebugMessage) << "slope: " << new_slope << std::endl;
        if( new_val <= val + mu*new_stepsize*base_slope && fabs(new_slope) <= -sigma*base_slope ) {
          vw_out(DebugMessage) << "*** ACCEPTING! ***" << std::endl;
          return new_pos;
        }
        else if( new_slope <= 0 ) {
          if( new_val >= val ) {
            vw_out(DebugMessage) << "Too large!" << std::endl;
            stepdelta *= 0.1;
          }
          else {
            vw_out(DebugMessage) << "Too small! (Retaining.)" << std::endl;
          ...
          vw_out(DebugMessage) << "Interpolating!" << std::endl;
          ...
          vw_out(DebugMessage) << "a=" << a << " b=" << b << " c=" << cur_slope << std::endl;
          double step = 0;
          if( fabs(a)*stepdelta < 1e-4*fabs(b) ) {
            vw_out(DebugMessage) << "Quadratic" << std::endl;
            step = -cur_slope/b;
          }
          else {
            vw_out(DebugMessage) << "Cubic" << std::endl;
            step = (-b+sqrt(b*b-4*a*cur_slope))/(2*a);
          }
          vw_out(DebugMessage) << "Before: " << step << std::endl;
          if( step < gamma*stepdelta ) step = gamma*stepdelta;
          else if( step > (1-gamma)*stepdelta ) step = (1-gamma)*stepdelta;
          vw_out(DebugMessage) << "After: " << step << std::endl;
          stepdelta = step;
        }
      }
    }

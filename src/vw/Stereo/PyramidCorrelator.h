...
namespace stereo {
  class PyramidCorrelator {
    ...
    bool is_good_subregion(ImageView<PixelMask<Vector2f> > const& disparity_map, ImageView<uint8> const& dmask, BBox2i box) {
      int num_disparity_pixels = count_valid_pixels(crop(disparity_map,box));
      int num_dmask_pixels = count_valid_pixels(crop(create_mask(dmask),box));

      // Must be >= because if num_disparity_pixels == num_dmask_pixels == 0, 
      // this is a GREAT subregion
      return num_disparity_pixels >= num_dmask_pixels * 0.1;
    }
    ...
    ImageView<PixelMask<Vector2f> > do_correlation(std::vector<ImageView<ChannelT> > left_pyramid,
                                                   ...
                                                   std::vector<ImageView<uint8> > dmask,
      ...
      for (int n = m_pyramid_levels - 1; n >=0; --n) {
        ...
        if (n == m_pyramid_levels - 1) {
          disparity_map.set_size(left_pyramid[n].cols(), left_pyramid[n].rows());
        } else {
          // Upscale disparity map from previous level
          disparity_map = copy(2*crop(edge_extend(upsample_by_two(disparity_map), ZeroEdgeExtension()), 
        ...
        if (count_valid_pixels(create_mask(dmask[n])) != 0) {
          if (is_good_subregion(disparity_map, dmask[n], BBox2i(0, 0, left_pyramid[n].cols(), left_pyramid[n].rows()))) {
            nominal_blocks = subdivide_bboxes(disparity_map, dmask[n], BBox2i(0, 0, left_pyramid[n].cols(), left_pyramid[n].rows()));
          } else {
            nominal_blocks.push_back(BBox2i(0, 0, left_pyramid[n].cols(), left_pyramid[n].rows()));
          }
          search_ranges = compute_search_ranges(disparity_map, dmask[n], nominal_blocks, m_initial_search_range / pow(2, n));
        }
        ...
        if (n == 0) {
          ...
          if (n == m_pyramid_levels - 1) {
            ...
            ImageView<PixelMask<Vector2f> > disparity_map_diff = invert_mask(intersect_mask(disparity_map, disparity_map_clean));
            disparity_map = disparity_mask(create_mask(apply_mask(disparity_map_clean) + apply_mask(disparity_map_diff)), 
                                           left_masks[n], right_masks[n]);
          }
        }
        ...
        if (m_debug_prefix.size() > 0) {
          ...
          std::ostringstream current_level;
          current_level << n;
          write_image(m_debug_prefix+current_level.str()+"-mask.jpg", dmask[n]);
        }
      }
  ...
  public:
    ...
    PyramidCorrelator(BBox2 initial_search_range,
      ...
      m_debug_prefix = "";
      m_min_subregion_dim = 3 * m_kernel_size[0];
    }
    ...
    ImageView<PixelMask<Vector2f> > operator() (ImageViewBase<ViewT> const& left_image,
      ...
      std::vector<ImageView<uint8> > dmask(m_pyramid_levels);
      ...
      // Build the dmask (a bitmask that valid for all area that 
      // could possibly be correlated with our initial search range)
      std::vector<vw::uint32> x_kernel(m_initial_search_range.width()+1), y_kernel(m_initial_search_range.height()+1);
      std::fill(x_kernel.begin(), x_kernel.end(), 1);
      std::fill(y_kernel.begin(), y_kernel.end(), 1);
      Vector2i kernel_origin = m_initial_search_range.max();
      dmask[0] = pixel_cast<vw::uint8>(threshold(separable_convolution_filter(threshold(right_masks[0],0,0,1), x_kernel, y_kernel, kernel_origin[0], kernel_origin[1], ZeroEdgeExtension()) * threshold(left_masks[0],0,0,1), 
                                                 (m_initial_search_range.width()+1)*(m_initial_search_range.height()+1)-1, 0, 255)); 
      ...
      for (int n = 1; n < m_pyramid_levels; ++n) {
        ...
        dmask[n] = subsample_mask_by_two(dmask[n-1]);
      }
      ...
      for (int n = 0; n < m_pyramid_levels; ++n) {
        ...
        dmask[n] = apply_mask(edge_mask(dmask[n], 0, mask_padding),0);
      }

      return do_correlation(left_pyramid, right_pyramid, left_masks, right_masks, dmask, preproc_filter);
    }

...
namespace rewrite {
  ...
  class PyramidCorrelationView : public ImageViewBase<PyramidCorrelationView<Image1T,Image2T,PreFilterT> > {
    ...
    void subdivide_regions( ImageView<PixelMask<Vector2i> > const& disparity,
                            BBox2i const& current_bbox,
                            std::list<SearchParam>& list ) const {
      // Is this a small enough section to append?
      if ( prod(current_bbox.size()) <= 16 ||
           current_bbox.width() < 2 || current_bbox.height() < 2 ){

        PixelAccumulator<EWMinMaxAccumulator<Vector2i> > accumulator;
        for_each_pixel( crop(disparity,current_bbox), accumulator );

        if ( !accumulator.is_valid() ) return;

        list.push_back( SearchParam( current_bbox, BBox2i(accumulator.minimum(),
                                                          accumulator.maximum()) ) );
        return;
      }

      // Start cutting
      BBox2i sub1 = current_bbox, sub2 = current_bbox;
      if ( current_bbox.width() > current_bbox.height() ) {
        sub1.max().x() = sub1.min().x() + current_bbox.width()/2;
        sub2.min().x() += current_bbox.width()/2;
      } else {
        sub1.max().y() = sub1.min().y() + current_bbox.height()/2;
        sub2.min().y() += current_bbox.height()/2;
      }
      subdivide_regions( disparity, sub1, list );
      subdivide_regions( disparity, sub2, list );
    }
  ...
  public:
    ...
    inline prerasterize_type prerasterize(BBox2i const& bbox) const {
      ...
      for ( int32 level = max_pyramid_levels; level >= 0; level--) {
        ...
        if ( level != 0 ) {
          zones.clear();
          subdivide_regions( disparity, bounding_box(disparity), zones );
          scaling >>= 1;
          ...
          BOOST_FOREACH( SearchParam& zone, zones ) {
            zone.first *= 2;
            zone.second *= 2;
            zone.second.max() += Vector2i(1,1);
            zone.second.expand(1); // This is practically required. Our
            // correlation will fail if the
            // search has only one solution. I'm
            // unsure if this is the correct
            // operation.

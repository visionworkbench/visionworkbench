...
namespace vw {
  namespace stereo {
	...
	GammaMixtureComponent(ImageViewBase<ImageT> const& image, 
			      ...
			      PrecisionT k_0, PrecisionT theta_0, PrecisionT pk_min, PrecisionT ptheta_min) : image_r(image.impl()), image_c(window_size(0), window_size(1)),
	_k(k_0), k0(k_0), 
	...
	*/
	...
	if(s <= 0) { // this is to keep things well conditioned; the equation log(k) - digamma(k) = s cannot be solved for s <= 0
	  s = 1e-6;
	  //cout << "s negative! K became too low!" << endl;
	}
	...
	*/
	...
	if(l_likelihood_old /*+ 1e-10*/ < l_likelihood) {
	  //cout << "LIKELIHOOD WENT UP IN Gamma by " << l_likelihood - l_likelihood_old<< endl;
	  //cout << "reverting k: " << _k << " -> " << k_last << endl;
	  //cout << "reverting theta: " << _theta << " -> " << theta_last << endl;
	  //cout << "s = " << s << endl;
	  _k = k_last;
	  _theta = theta_last;
	  l_likelihood = -sum_of_pixel_values(w*((_k-1)*log(image_c) - 			 
	...
	if(l_likelihood > DBL_MAX || l_likelihood != l_likelihood) {
	  cout << "l is bad: " << l_likelihood << endl;
	  cout << "k: " << _k << endl;
	  cout << "theta: " << _theta << endl;
	  cout << "s: " << s << endl;
	  cout << "mu: " << mu << endl;
	  cout << "mu_log: " << mu_log << endl;
	  cout << "sum_weights_adj: " << sum_weights_adj << endl;
	}
	*/
      }
	...
	cout << prefix << "k = " << _k << endl;
	cout << prefix << "theta = " << _theta << endl;
      }
	...
	return _theta;
      ...
      ImageView<PrecisionT> & weights() { 
	return w;
      }
      
      ImageView<PrecisionT> const& weights() const { 
	return w;
      }
      
      ImageView<PrecisionT> const& prob() const {
	return p;
      }

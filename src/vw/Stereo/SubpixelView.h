...
namespace stereo {
  ...
  //######################################################################
  // New subpixel classes
  // -TODO: Modify one of these classes so code can be reused!  Only a few lines differ.


  /// An image view for performing image correlation using affine sub-pixel correlation.
  template <class PreprocFilterT, class ImageT1, class ImageT2, class ImageTD>
  class AffineSubpixelView : public ImageViewBase<AffineSubpixelView<PreprocFilterT, ImageT1, ImageT2, ImageTD> > {
  ...
  public:
    ...
    AffineSubpixelView(ImageViewBase<ImageTD> const& disparity_map,
                        ImageViewBase<ImageT1> const& left_image,
                        ImageViewBase<ImageT2> const& right_image,
                        PreFilterBase<PreprocFilterT> const& preproc_filter,
                        Vector2i const& kernel_size,
      ...
      VW_ASSERT( m_disparity_map.cols() == m_left_image.cols() &&
                 m_disparity_map.rows() == m_left_image.rows(),
                 ArgumentErr() << "AffineSubpixelView::AffineSubpixelView():  Disparity image must match left image.\n");

      VW_ASSERT((m_left_image.channels() == 1) && (m_left_image.planes() == 1) &&
                (m_right_image.channels() == 1) && (m_right_image.planes() == 1),
                ArgumentErr() << "AffineSubpixelView::AffineSubpixelView(): multi-channel, multi-plane images are not supported.\n");
    ...
    inline pixel_type operator()(float /*x*/, float /*y*/, int32 /*p*/ = 0) const {
      vw_throw(NoImplErr() << "AffineSubpixelView::operator() is not yet implemented.");
      return PixelMask<Vector2f>(); // Never reached
    }
  ...
  template <class PreprocFilterT, class ImageT1, class ImageT2, class DisparityT>
  AffineSubpixelView<PreprocFilterT, ImageT1, ImageT2, DisparityT>
  affine_subpixel( ImageViewBase<DisparityT> const& disparity_map,
                   ...
                   Vector2i const& kernel_size,
                   int max_pyramid_levels = 2 ) {
    typedef AffineSubpixelView<PreprocFilterT,ImageT1,ImageT2,DisparityT> result_type;

    return result_type( disparity_map.impl(), left_image.impl(),
                        right_image.impl(), filter.impl(), kernel_size,
                        max_pyramid_levels );
  }
  // End of components for Affine subpixel
  ...
  /// An image view for performing image correlation using Lucas-Kanade sub-pixel correlation.
  template <class PreprocFilterT, class ImageT1, class ImageT2, class ImageTD>
  class LKSubpixelView : public ImageViewBase<LKSubpixelView<PreprocFilterT, ImageT1, ImageT2, ImageTD> > {
  ...
  public:
    typedef PixelMask<Vector2f> pixel_type;
    typedef pixel_type          result_type;
    typedef ProceduralPixelAccessor<LKSubpixelView> pixel_accessor;

     LKSubpixelView(ImageViewBase<ImageTD> const& disparity_map,
                    ImageViewBase<ImageT1> const& left_image,
                    ImageViewBase<ImageT2> const& right_image,
                    PreFilterBase<PreprocFilterT> const& preproc_filter,
                    Vector2i const& kernel_size,
      ...
      VW_ASSERT( m_disparity_map.cols() == m_left_image.cols() &&
                 m_disparity_map.rows() == m_left_image.rows(),
                 ArgumentErr() << "LKSubpixelView::LKSubpixelView():  Disparity image must match left image.\n");

      VW_ASSERT((m_left_image.channels() == 1) && (m_left_image.planes() == 1) &&
                (m_right_image.channels() == 1) && (m_right_image.planes() == 1),
                ArgumentErr() << "LKSubpixelView::LKSubpixelView(): multi-channel, multi-plane images are not supported.\n");
    ...
    inline pixel_type operator()(float /*x*/, float /*y*/, int32 /*p*/ = 0) const {
      vw_throw(NoImplErr() << "LKSubpixelView::operator() is not yet implemented.");
      return PixelMask<Vector2f>(); // Never reached
    }
  ...
  template <class PreprocFilterT, class ImageT1, class ImageT2, class DisparityT>
  LKSubpixelView<PreprocFilterT, ImageT1, ImageT2, DisparityT>
  lk_subpixel( ImageViewBase<DisparityT> const& disparity_map,
                   ...
                   Vector2i const& kernel_size,
                   int max_pyramid_levels = 2 ) {
    typedef LKSubpixelView<PreprocFilterT,ImageT1,ImageT2,DisparityT> result_type;

    return result_type( disparity_map.impl(), left_image.impl(),
                        right_image.impl(), filter.impl(), kernel_size,
                        max_pyramid_levels );
  }
  // End of components for Lucas-Kanade subpixel

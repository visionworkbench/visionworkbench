...
ImageView<PixelT> vw::platefile::ToastPlateManager<PixelT>::composite_child_tile(int &num_composited, 
  ...
  if( x==-1 ) {
    if( y==-1 ) {
      return composite_child_tile(num_composited, num_tiles-1, num_tiles-1, level,
                                  read_transaction_id, write_transaction_id, this_transaction_only);
    }
    if( y==num_tiles ) {
  ...
  bool found_record = true;
  try {
    rec = m_platefile->read_record(x, y, level, write_transaction_id, true);  // exact_transaction_match = true
  } catch (TileNotFoundErr &e) {
    found_record = false;
  }
  ...
  ImageView<PixelT> tile;
  if (found_record && rec.status() == INDEX_RECORD_VALID) {
    m_platefile->read(tile, x, y, level, write_transaction_id, true);
    num_composited = 1;
  }

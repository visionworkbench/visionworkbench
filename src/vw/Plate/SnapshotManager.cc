...
namespace platefile {
  ...
  void snapshot_helper(int current_col, 
                       int current_row, 
                       int current_level, 
    ...
    try {
      tile_records = platefile->search_by_location(current_col, current_row, current_level,
    ...
    // Insert the TileHeaders for this level into the parent_tiles
    // map.  Elements will be inserted in order, and any duplicate
    // transaction_ids will be overwritten by new tile records.
    ...
    if (current_level == target_level) {
      ...
      ImageView<PixelT> composite_tile(platefile->default_tile_size(),
                                       platefile->default_tile_size());

      std::cout << "COMPOSITING " << current_col << " " << current_row 
      ...
      for (std::map<int32, TileHeader>::reverse_iterator iter = composite_tiles.rbegin(); 
        ...
        if ( is_opaque(new_tile) && 
             iter == composite_tiles.rbegin() && 
             current_hdr.level() == target_level) 
          break;
      ...
      if (num_composited > 1) {
        // -- debug
        // std::cout << "Compositing " << header_current_hdr.col() << " " 
        //           << header_current_hdr.row() << " @ " << header_current_hdr.level() << "  [ "
        //           << tiles_at_location.size() << "]\n";
        //---

        platefile->write_update(composite_tile, 
      ...
      // There are four possible children that we can explore for more
      ...
      for (int new_row = current_row*2; new_row < current_row*2+2; ++new_row) {
        for (int new_col = current_col*2; new_col < current_col*2+2; ++new_col) {
          ...
          if (parent_region.intersects(target_region)) {
            snapshot_helper<PixelT>(new_col, new_row, current_level+1,
                                    composite_tiles, platefile, target_region, target_level, 
                                    start_transaction_id, end_transaction_id, 
                                    write_transaction_id);
          }
        }
...
void vw::platefile::SnapshotManager<PixelT>::snapshot(int level, BBox2i const& tile_region, 
  ...
  for ( std::list<BBox2i>::iterator region_iter = tile_workunits.begin(); 
    ...
    std::map<int32, TileHeader> composite_tiles;
    snapshot_helper<PixelT>(0, 0, 0, composite_tiles, m_platefile, *region_iter, level, 
                            start_transaction_id, end_transaction_id, write_transaction_id);
    return;
...
void vw::platefile::SnapshotManager<PixelT>::full_snapshot(int start_transaction_id, 
  ...
  for (int level = 0; level < m_platefile->num_levels(); ++level) {    
    ...
    for ( std::list<BBox2i>::iterator region_iter = workunits.begin(); 
      ...
      std::cout << "\n\nSNAPSHOTTING : " << level << " " << *region_iter  << "\n";

...
namespace vw {
  ...
  class ThreadedBlockWriter {
    ...
    public:
      ...
      m_progress_callback(progress_callback,0.0,1.0/float(total_num_blocks)) {}
    ...
    // Conditional class to determine if a write task can be added
    class WriteConditional {
      boost::weak_ptr<OrderedWorkQueue> m_write_work_queue; // So this class can't maintain ownership

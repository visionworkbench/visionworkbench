...
#ifndef __VW_IMAGE_ERODE_VIEW_H__
#define __VW_IMAGE_ERODE_VIEW_H__
...
inline CropView<ImageView<typename ImageT::pixel_type> >
          makeHiddenCroppedView( ImageT &imageToCrop, BBox2i const& bbox )
{
...
*/
template <class ImageT>
class ErodeView : public ImageViewBase< ErodeView<ImageT> >
{
public: // Definitions
  ...
  typedef ProceduralPixelAccessor<ErodeView<ImageT> > pixel_accessor;
...
public: // Functions
  ...
  ErodeView( ImageViewBase<ImageT>       const& under_image,
  ...
  inline int32 cols  () const { return m_under_image.cols(); } ///< Get the number of rows
  inline int32 rows  () const { return m_under_image.rows(); } ///< Get the number of columns
  inline int32 planes() const { return 1; }                    ///< Get the number of planes TODO!
...
private: // Definitions
  ...
  typedef std::map<int32,int32> map_type; ///< Sparse container
...
private: // Functions
  ...
  const map_type& blobRow(const int32 row) const {return (*m_data)[row];}
        map_type& blobRow(const int32 row)       {return (*m_data)[row];}
...
inline ErodeView<SourceT> applyErodeView( ImageViewBase<SourceT>      const& src,
                                          std::list<blob::BlobCompressed> const& blobList) {
...
template <class ImageT>
ErodeView<ImageT>::ErodeView( ImageViewBase<ImageT>       const& under_image,
                              std::list<blob::BlobCompressed> const& blobList)
...
template <class ImageT>
typename ErodeView<ImageT>::result_type ErodeView<ImageT>::operator()( int32 i, int32 j, int32 p) const
{
  ...
  {
    int32 startIndex = it->second; // Get the starting index of this section
    if ( i < startIndex )
    {
...
void ErodeView<ImageT>::absorb(blob::BlobCompressed const& newBlob)
{
  ...
  for (int row=0; row<numRows; ++row)
  {
    ...
    std::list<int32>::const_iterator iterStart, iterEnd;
         ...
         ++iterStart, ++iterEnd)
    {
      ...
      while (i!= stopIter)
      {
       ...
       std::pair<int32,int32> newSegment(segmentEnd, segmentStart);
...
} // End function "void absorb(const blob::BlobCompressed &newBlob)"
...
template <class ImageT>
typename ErodeView<ImageT>::prerasterize_type ErodeView<ImageT>::prerasterize( BBox2i const& bbox ) const
{
...
template <class ImageT>
template <class DestT>
void ErodeView<ImageT>::rasterize( DestT const& dest, BBox2i const& bbox ) const
{

...
//#include <boost/graph/breadth_first_search.hpp> //for printing graph
  ...
  namespace ip{
	...
	template<typename RandomAccessIterT>
	  unsigned operator() (RandomAccessIterT file_beg, RandomAccessIterT file_end, unsigned unused_argument)
	  {
	    ...
	    //std::numeric_limits<T>::max() in place of DBL_MAX
	    std::vector<double> mins;
	    std::vector<double> maxes;
	    mins.assign(k,DBL_MAX);
	    maxes.assign(k,-DBL_MAX);
    ...
    template<typename RandomAccessIterT>
      ...
      typedef graph_traits< adjacency_list<vecS, vecS, undirectedS > >::vertex_descriptor Vertex;
      //      typedef graph_traits< adjacency_list<vecS, vecS, undirectedS > >::edge_descriptor Edge;

      //The underlying graph structure:
      typedef adjacency_list<vecS, vecS, undirectedS,
      property<vertex_discriminator_t, unsigned,
      property<vertex_record_t, record_t,
      property<vertex_LOSON_t, Vertex,
      property<vertex_HISON_t, Vertex,
      property<vertex_LORANGE_t, range_t,
      property<vertex_HIRANGE_t, range_t > > > > > > > BGL_KDTree;
      ...
      typename property_map<BGL_KDTree, vertex_record_t>::type m_record_map;
      typename property_map<BGL_KDTree, vertex_discriminator_t>::type m_discriminator_map;
      typename property_map<BGL_KDTree, vertex_LOSON_t>::type m_LOSON_map;
      typename property_map<BGL_KDTree, vertex_HISON_t>::type m_HISON_map;
      typename property_map<BGL_KDTree, vertex_LORANGE_t>::type m_LORANGE_map;
      typename property_map<BGL_KDTree, vertex_HIRANGE_t>::type m_HIRANGE_map;
	...
	m_record_map = get(vertex_record, m_kdTree);
	m_discriminator_map = get(vertex_discriminator, m_kdTree);
	m_LOSON_map = get(vertex_LOSON, m_kdTree);
	m_HISON_map = get(vertex_HISON, m_kdTree);
	m_LORANGE_map = get(vertex_LORANGE, m_kdTree);
	m_HIRANGE_map = get(vertex_HIRANGE, m_kdTree);
      }
			  ...
			  int previous_disc, unsigned depth)
	{
	  ...
	  Edge ed;
	  bool inserted;
	  tie(ed, inserted) = add_edge(P,lo,m_kdTree);
	  tie(ed, inserted) = add_edge(P,hi,m_kdTree);
	  */
	...
	if(P == m_NIL){
	  ...
	  //range_t lo_range(m_k, -DBL_MAX);
	  //range_t hi_range(m_k, DBL_MAX);
	  range_t lo_range(m_k, m_NEGATIVE_INFINITY);
	  range_t hi_range(m_k, m_POSITIVE_INFINITY);
	  ...
	  Edge ed;
	  bool inserted;
	  tie(ed, inserted) = add_edge(new_vertex, m_NIL,m_kdTree);
	  tie(ed, inserted) = add_edge(new_vertex, m_NIL,m_kdTree);
	  */
	...
	if (!m_priority_queue.empty()){
	  tie(dummy, distance_to_mth_best) = m_priority_queue.top();
	}else{
	...
	return true;
      ...
      //
      // TODO: all uses of DBL_MAX should be replaced with a C implementation of infinity!

...
namespace vw { namespace ip {
  /*
  // Template HarrisInterestView?
  /// Harris interest measure uses the image gradients.
  template <class SrcT> struct InterestViewTraits<SrcT, HarrisInterestView> {
    typedef ImageView<typename SrcT::pixel_type> gradient_type;
    typedef typename DefaultMagT<gradient_type>::type mag_type;
    typedef typename DefaultOriT<gradient_type>::type ori_type;
    typedef SrcT interest_type;
  };
  ...
  */
  ...
  namespace detail {
    template <class SrcT>
    struct LogInterestHelper {
      typedef typename DefaultKernelT<typename SrcT::pixel_type>::type channel_type;
      typedef UnaryPerPixelView<ConvolutionView<SrcT, ImageView<channel_type>, ConstantEdgeExtension>, vw::ValArgProductFunctor<channel_type> > type;
    };
  }

  /// Laplacian of Gaussian interest image view
  template <class SrcT>
  class LoGInterestView : public detail::LogInterestHelper<SrcT>::type {
  public:
    LoGInterestView(ImageInterestData<SrcT,LoGInterestView<SrcT> > const& data,
                float scale = 1.0)
      : detail::LogInterestHelper<SrcT>::type(scale * laplacian_filter(data.source())) {}

    LoGInterestView(ImageViewBase<SrcT> const& image, float scale = 1.0)
      : detail::LogInterestHelper<SrcT>::type(scale * laplacian_filter(image)) {}
  };

  /// LoG interest functor
  class LoGInterest {
  public:
    template <class SrcT> struct ViewType { typedef LoGInterestView<SrcT> type; };

    LoGInterest() {}

    template <class ViewT>
    inline ViewT operator() (ImageViewBase<ViewT> const& source, float scale = 1.0) const {
      return LoGInterestView<ViewT>(source, scale);
    }

    // TODO: this should return something
    template <class DataT>
    inline void operator() (DataT& data, float scale = 1.0) const {
      data.set_interest(LoGInterestView<typename DataT::source_type>(data.source(), scale));
    }
  };

  /// Type traits for LoG interest
  template <> struct InterestPeakType <LoGInterest> { static const int peak_type = IP_MINMAX; };

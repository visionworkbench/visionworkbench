...
namespace vw {
  ...
  class DiskImageView : public ImageViewBase<DiskImageView<PixelT> >
  {
    ...
    class BlockGenerator {
      DiskImageView *m_view_ptr;
      BBox2i m_bbox;
    public:
      typedef ImageView<PixelT> value_type;

      BlockGenerator( DiskImageView *view_ptr, BBox2i bbox )
        : m_view_ptr( view_ptr ), m_bbox( bbox ) {}
      
      size_t size() const {
        return m_bbox.width() * m_bbox.height() * m_view_ptr->planes() * sizeof(PixelT);
      }
    ...
    void initialize() {
      ...
      for( int y=0; y<=maxy; ++y ) {
        for( int x=0; x<=maxx; ++x ) {
          ...
          bbox.crop( view_bbox );
          Cache::Handle<BlockGenerator> handle = m_cache.insert( BlockGenerator( this, bbox ) );
  ...
  public:
    ...
    DiskImageView( std::string const& filename )
      : r( DiskImageResource::open( filename ) ),
        m_filename( filename ), m_cache(Cache::system_cache()),
    ...
    DiskImageView( std::string const& filename, Cache& cache )
      : r( DiskImageResource::open( filename ) ),
        m_filename( filename ), m_cache(cache),
        m_block_size( r->native_read_block_size() )
    { initialize(); }

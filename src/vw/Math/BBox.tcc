...
BBoxBase<BBoxT, RealT, DimN>::BBoxBase() {
...
void BBoxBase<BBoxT, RealT, DimN>::grow( VectorBase<VectorT> const& point ) {
...
void BBoxBase<BBoxT, RealT, DimN>::crop( BBoxBase<BBoxT1, RealT1, DimN1> const& bbox ) {
...
template <class BBoxT, class RealT, size_t DimN>
template <class VectorT>
bool BBoxBase<BBoxT, RealT, DimN>::contains( const VectorBase<VectorT> &point ) const {
...
template <class BBoxT, class RealT, size_t DimN>
template <class BBoxT1, class RealT1, size_t DimN1>
bool BBoxBase<BBoxT, RealT, DimN>::contains( const BBoxBase<BBoxT1, RealT1, DimN1> &bbox ) const {
  using namespace vector_containment_comparison;
...
template <class BBoxT, class RealT, size_t DimN>
template <class BBoxT1, class RealT1, size_t DimN1>
bool BBoxBase<BBoxT, RealT, DimN>::intersects( const BBoxBase<BBoxT1, RealT1, DimN1>& bbox ) const {
  VW_ASSERT(m_min.size() == 0 || bbox.min().size() == m_min.size(), 
            ArgumentErr() << "BBox must have dimension " << m_min.size() << ".");
  for( size_t i=0; i<m_min.size(); ++i ) {
    if( m_min[i] >= bbox.max()[i] ||
...
template <class BBoxT, class RealT, size_t DimN>
bool BBoxBase<BBoxT, RealT, DimN>::empty() const {
  for( size_t i=0; i<m_min.size(); ++i )
...
template <class BBoxT, class RealT, size_t DimN>
Vector<RealT, DimN> BBoxBase<BBoxT, RealT, DimN>::size() const {
...
template <class BBoxT, class RealT, size_t DimN>
Vector<RealT, DimN> BBoxBase<BBoxT, RealT, DimN>::center() const {
  if (empty()) return Vector<RealT, DimN>(); // Bug fix for underflow/overflow
...
template <class BBoxT, class RealT, size_t DimN>
RealT BBoxBase<BBoxT, RealT, DimN>::width() const { return impl().width(); }

template <class BBoxT, class RealT, size_t DimN>
RealT BBoxBase<BBoxT, RealT, DimN>::height() const { return impl().height(); }

template <class BBoxT, class RealT, size_t DimN>
RealT BBoxBase<BBoxT, RealT, DimN>::depth() const { return impl().depth(); }

template <class BBoxT, class RealT, size_t DimN>
void BBoxBase<BBoxT, RealT, DimN>::expand( RealT offset ) {
...
template <class BBoxT, class RealT, size_t DimN>
void BBoxBase<BBoxT, RealT, DimN>::contract( RealT offset ) {
...
BBoxBase<BBoxT, RealT, DimN>& BBoxBase<BBoxT, RealT, DimN>::operator*=( ScalarT s ) {
...
template <class BBoxT, class RealT, size_t DimN>
template <class VectorT>
BBoxBase<BBoxT, RealT, DimN>& BBoxBase<BBoxT, RealT, DimN>::operator+=( VectorBase<VectorT> const& v ) {
  if (empty()) return *this; // Bug fix for underflow/overflow
  m_min += v;
  m_max += v;
  return *this;
...
template <class RealT, size_t DimN>
template <class BBoxT1, class RealT1, size_t DimN1>
BBox<RealT, DimN>::BBox( BBoxBase<BBoxT1, RealT1, DimN1> const& bbox )
  : BBoxBase<BBox<RealT, DimN>, RealT, DimN>( bbox.min(), bbox.max() ) {}

template <class RealT, size_t DimN>
template <class BBoxT1, class RealT1, size_t DimN1>
BBox<RealT, DimN>& BBox<RealT, DimN>::operator=( BBoxBase<BBoxT1, RealT1, DimN1> const& bbox ) {
  this->min() = bbox.min();
  this->max() = bbox.max();
  return *this;
}
...
BBox<RealT>::BBox( RealT minx, RealT miny, RealT width, RealT height )
...
template <class RealT>
BBox<RealT>::BBox( Vector<RealT, 0> const& min, Vector<RealT, 0> const& max )
  : BBoxBase<BBox<RealT, 0>, RealT, 0>(min, max) {}


template <class RealT>
BBox<RealT>::BBox( RealT minx, RealT miny, RealT minz, RealT width, RealT height, RealT depth )
  : BBoxBase<BBox<RealT, 0>, RealT, 0>(
      Vector<RealT,3>(minx,miny,minz),
      Vector<RealT,3>(minx+width,miny+height,minz+depth) ) {}
...
template <class RealT>
template <class BBoxT1, class RealT1, size_t DimN1>
BBox<RealT>::BBox( BBoxBase<BBoxT1, RealT1, DimN1> const& bbox )
  : BBoxBase<BBox<RealT, 0>, RealT, 0>( bbox.min(), bbox.max() ) {}

template <class RealT>
template <class BBoxT1, class RealT1, size_t DimN1>
BBox<RealT, 0>& BBox<RealT, 0>::operator=( BBoxBase<BBoxT1, RealT1, DimN1> const& bbox ) {
  this->min() = bbox.min();
  this->max() = bbox.max();
  return *this;
}

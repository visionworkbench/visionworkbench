...
// with no outlier filtering. See also find_3D_transform().
void find_3D_transform_no_outliers(vw::Matrix<double> const & in_vec, 
  ...
  if (transform_type != "similarity" && transform_type != "rigid" &&
    ...
    vw_throw(vw::ArgumentErr() << "find_3D_transform_no_outliers: Expecting to compute a "
...
void find_3D_transform_no_outliers(std::vector<vw::Vector3> const & in_vec,
  ...
  // Create vw matrices out of the inputs, with these vectors as rows
  ...
  // Call the interface with matrices
  return find_3D_transform_no_outliers(in, out, rotation, 
                                       translation, scale, transform_type);
}
...
struct Similarity3DFittingFunctor {
  ...
  result_type operator()(std::vector<vw::Vector3> const& in_vec,
    ...
    // TODO(oalexan1): Eliminate the matrix format altogether
    int num_pts = in_vec.size();
    vw::Matrix<double> in, out;
    in.set_size(3, num_pts);
    out.set_size(3, num_pts);
    ...
    vw::Vector<double, 3>    translation;
    double                   scale;
    find_3D_transform_no_outliers(in, out, rotation, translation, scale, m_transform_type);
...
void find_3D_transform(vw::Matrix<double> const & in, 
  ...
  if (!filter_outliers) {
    find_3D_transform_no_outliers(in, out, rotation, translation, scale, transform_type);
    return;
  }

...
public: // Functions
  ...
  void load_match_data(MatrixBase<MatrixT> const& features, FLANN_DistType dist_type) {
    ...
    construct_index((void*)&m_features_cast(0,0), m_features_cast.rows(), 
  ...
  size_t knn_search(MatrixBase<MatrixT> const& query, // Values we are looking for
    ...
    int flann_found = knn_search_help((void*)&query_cast(0,0), query_cast.rows(), 
                                      query_cast.cols(),
                                      indices, dists, knn);
  ...
  size_t knn_search(VectorBase<VectorT> const& query, // Values we are looking for
    ...
    int flann_found = knn_search_help((void*)&query_cast[0], 1, query_cast.size(), 
                                      indices, dists, knn);

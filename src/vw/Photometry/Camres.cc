...
vw::photometry::save_exposure_images(std::vector<std::string> output_files,
  ...
  if ( stat(input_file, &file_stat) ) {
    ...
    save_exposure_images(output_files, input_files,
                         image_response, file_stat.st_mtime);
  } else {
...
vw::photometry::save_exposure_images(std::vector<std::string> output_files,
  ...
  for (size_t i = 0; i < input_files.size(); ++i) {
    ...
    tm_image = copy_mask(apply_mask(tm_image,0), image);
    write_georeferenced_image(output_files[i], tm_image, geo, TerminalProgressCallback("photometry","Processing:"));
    n++;
  }
  return n;
}

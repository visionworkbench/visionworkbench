...
vw::photometry::derivative_filenames(std::vector<std::string> input_files,
...
void vw::photometry::error_small_buffer(const char * file_name,
                                        unsigned entry_size) {
...
template <class T>
void vw::photometry::load_binary_file(T * ptr, size_t count,
                                      const char * filename) {
...
vw::photometry::save_binary_file(T * ptr, size_t count,
                                 const char * filename) {
...
Vector<uint8>
vw::photometry::load_inverse_weight(char * input_file, int num) {
...
vw::photometry::save_residual_histogram(char * output_file,
                        std::vector<std::string> input_files,
                        std::vector<std::string> uintrad_files,
...
Vector<float>
vw::photometry::save_logexp_histogram(char * output_file,
                      std::vector<std::string> output_files,
                      std::vector<std::string> input_files,
                      std::vector<std::string> radiance_files,
                      Vector<float> exposure_times,
  ...
  for (unsigned i = 0; i < input_files.size(); ++i) {
    ...
    for (unsigned k = 0; k < DYNAMIC_RANGE; ++k) buffer[k] = image_histogram(k);
    save_binary_file(buffer,DYNAMIC_RANGE,output_files[i].c_str());
    std::cout << "Logarithmic Exposure Histogram of "
              << input_files[i] << ": " << image_histogram << std::endl;
  ...
  save_binary_file(buffer,DYNAMIC_RANGE,output_file);
  std::cout << "Logarithmic Exposure Histogram of "
            << output_file << ": " << images_histogram << std::endl;

  return images_histogram;
}

float
vw::photometry::total_loglike(Vector<float> coeffs,
                              Vector<float> image_histogram,
                              Vector<float> image_response) {
  ...
  for (unsigned i = 0; i < DYNAMIC_RANGE; ++i) {
    ...
    std::cout << i << " " << sum_loglike << " " << coeffs(i)
              << " " << image_response(i) << " " <<  image_histogram(i)
              << " " << lgamma(image_response(i)+1) << std::endl;
  }
  return sum_loglike;
}

void
vw::photometry::convert_real2image(std::string input_file,
  ...
  if (ub == DYNAMIC_RANGE-1 && lb == 0.0) {
    ...
    std::cout << input_file << " in [ " << lb << ", "
              << ub << "] => " << output_file << "." << std::endl;
...
void
vw::photometry::convert_real2images(std::vector<std::string> input_files,
                                    std::vector<std::string> output_files,
                                    float ub, float lb) {
  for (unsigned i = 0; i < input_files.size(); ++i) {
    std::cout << " " << i << "th image: " ;
...
vw::photometry::convert_real2images(std::vector<std::string> input_files,
  ...
  if (bNormalized) {
    for (unsigned i = 0; i < input_files.size(); ++i) {
      ...
      if (lb > lo) lb = lo;
      if (ub < hi) ub = hi;
      std::cout << "Range of the " << i << "th image "
                << input_files[i] << " : (" << lo << ","
                << hi << ")" << std::endl;
    }

    std::cout << "Range of all image: (" << lb << ","
              << ub << ")" << std::endl;
  } else {
    lb =0;
    ub = DYNAMIC_RANGE-1;

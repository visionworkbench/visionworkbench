...
void do_mosaic(po::variables_map const& vm) {
  ...
  if( output_metadata == "kml" ) {
    ...
    tmp = new UniviewQuadTreeGenerator<PixelT>( output_file_name, composite );
    quadtree = tmp;
  ...
  if(output_metadata == "uniview") { 
    ...
    conf << "TextureCallstring=Generated by the NASA Vision Workbench image2geotree tool.\n";
...
int main(int argc, char **argv) {
  ...
  output_options.add_options()
    ...
    ("channel-type", po::value<std::string>(&channel_type_str)->default_value("uint8"), "Output channel type. One of [uint8, uint16, int16, float]")
  ...
  if(vm.count("channel-type")) {
    ...
    else if(channel_type_str == "uint16") channel_type_str = VW_CHANNEL_UINT16;

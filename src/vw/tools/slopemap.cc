...
template <class ImageT>
Vector2 gradient_aspect_from_dtheta_dphi(int x, int y, double dtheta, double dphi, DiskImageView<ImageT> img, GeoReference GR) {
	double rho=norm_2(pixel_to_cart(Vector2(x,y),img,GR));
	Vector2 lonlat=GR.pixel_to_lonlat(Vector2(x,y));
	double lat=lonlat(1);
	double lon=lonlat(0);
	double phi=lon/180.0*pi;
	double theta=(-lat+90.0)/180.0*pi;
	return gradient_aspect_from_dtheta_dphi(rho, theta, phi, dtheta, dphi, pixel_to_cart(Vector2(x,y), img, GR));
}
...
desc.add_options()
	...
	("algorithm", po::value<std::string>(&algorithm_string)->default_value("moduneven"), "Choose an algorithm to calculate slope/aspect from [ horn, fh, sa, planefit ]. Horn: Horn's algorithm; FH: Fleming & Hoffer's (rook's case); SA: Sharpnack & Akin's (queen's case)")

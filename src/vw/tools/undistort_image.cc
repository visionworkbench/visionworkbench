...
void undistort_image(const PinholeModel                           &camera_model,
  ...
  for (int r=0; r<height_in; ++r) {
    out_loc  = lens_ptr->undistorted_coordinates(camera_model, Vector2(0,          r));
    output_area.grow(out_loc);
    out_loc  = lens_ptr->undistorted_coordinates(camera_model, Vector2(width_in-1, r));
    output_area.grow(out_loc);
  }
  for (int c=0; c<width_in; ++c) {
    out_loc  = lens_ptr->undistorted_coordinates(camera_model, Vector2(c,           0));
    output_area.grow(out_loc);
    out_loc  = lens_ptr->undistorted_coordinates(camera_model, Vector2(c, height_in-1));
    output_area.grow(out_loc);
  }
  ...
  for (int r=0; r<image_out.rows(); ++r) {
    for (int c=0; c<image_out.cols(); ++c) {
      ...
      Vector2 in_loc = lens_ptr->distorted_coordinates(camera_model, Vector2(c,r)+offset);

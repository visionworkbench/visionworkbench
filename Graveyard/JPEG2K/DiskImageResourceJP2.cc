...
namespace vw
{
  ...
  {
    if(m_additional_boxes)
      delete (JP2BoxList*)m_additional_boxes;
    if(m_additional_requirements)
      delete (JP2ReaderRequirementsList*)m_additional_requirements;
  }
...
// 	component->data = (int *) dst(0, 0, i);
  ...
  DiskImageResourceJP2::open(std::string const& filename)
  {
    ...
    m_additional_boxes = new JP2BoxList;
    m_additional_requirements = new JP2ReaderRequirementsList;
#if 0
...
//     coding_parameters.tile_size_on = true;
    ...
    { // Convert to jpx, and add GML if desired
      ...
      //jp2f.print();
      if(jp2f.convert_to_jpx() != 0)
        vw_throw(IOErr() << "DiskImageResourceJP2::write(): "
	       "failed to convert jp2 to backward-compatible jpx");
      if(jp2f.add_boxes(*((JP2BoxList*)m_additional_boxes)) != 0)
        vw_throw(IOErr() << "DiskImageResourceJP2::write(): "
	       "failed to add boxes to jpx");
      if(jp2f.add_requirements(*((JP2ReaderRequirementsList*)m_additional_requirements)) != 0)
        vw_throw(IOErr() << "DiskImageResourceJP2::write(): "
	       "failed to add requirements to jpx");
      //jp2f.print();

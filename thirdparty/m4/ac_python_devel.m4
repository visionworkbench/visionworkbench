...
# SYNOPSIS
#
#   AC_PYTHON_DEVEL([RUN-IF-OK], [RUN-IF-FAILED])
#
# DESCRIPTION
#
...
AC_DEFUN([AC_PYTHON_DEVEL],[
    ...
    if test x"$PYTHON" = x":"; then
        ...
        if test -n "$ac_distutils_result"; then
            AC_MSG_RESULT([no])
            PYTHON_VERSION=""
            m4_default([$2], [AC_MSG_ERROR([cannot import Python module "distutils".  Please check your Python installation. The error was: $ac_distutils_result])])
        else
            AC_MSG_RESULT([yes])
            ...
            if test ! "$pythonexists" = "yes"; then
              PYTHON_VERSION=""
              m4_default([$2], [AC_MSG_ERROR([Could not link test program to Python.])])
            else
                $1
            fi
            AC_LANG_POP([C])
